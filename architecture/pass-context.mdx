---
title: "PassContext: Capturing Pass Geometry in PES6"
sidebarTitle: "Pass Context"
description: "How PassContext captures passer and receiver memory pointers, reads player cell coordinates, computes discrete distance, and calculates real-world dot-product geometry."
---
...
// ball-to-passer direction vector.
    ...
    int* distSimple)
{
    ...
    *receiverY = *(BYTE*)(receiver + PesOffsets::PLAYER_CELL_Y);
    ...
    int dx = abs((int)*receiverX - (int)*passerX);
    int dy = abs((int)*receiverY - (int)*passerY);
    *distSimple = dx + dy;
    return true;
}
```
